...
def rag_ask(query, index, metadata, contents, bm25, k=3):
    """RAG问答，支持表格关键词检测及检索，使用混合检索"""
    ...
    # 取top-k规范文本结果
    norm_results = [r for r in results if r["metadata"]["type"] == "norm_text"][:k]

    # 如果需要表格，找距离<3的表格中距离最小的Top1，并添加到上下文
    matched_table = None
    table_result = None
    if want_table:
    ...
    print(f"\n选取Top-{k}规范文本构建Prompt:")
    for r in norm_results:
        print(f"  - {r['metadata']['content'][:50]}... (混合分数: {r['hybrid_score']:.4f})")
    ...
    for r in norm_results:
        knowledge_count += 1
        m = r["metadata"]
        context_str += f"背景知识 {knowledge_count} [规范文档] (来源: {m['source']}, 混合分数: {r['hybrid_score']:.4f}):\n{m['content']}\n\n"
...
if __name__ == "__main__":
    ...
    '''
    print("\n" + "="*60)
    print("测试: 混合检索（BM25 + 向量）")
    print("="*60)
    rag_ask("楼梯踏步的尺寸要求是什么？请提供相关表格", index, metadata, contents, bm25, k=3)
    '''

    print("\n" + "="*60)
    print("测试: 混合检索（BM25 + 向量）")
    print("="*60)
    rag_ask("金属夹芯板的屋面排水坡度是多少？请提供相关表格", index, metadata, contents, bm25, k=3)
